...
public class CheckInServiceImplTest {
	...
	public void setUp() {
		...
		ReflectionTestUtils.setField(checkInService, "passengerSummaries", passengerSummaries);
        ReflectionTestUtils.setField(checkInService, "metroCardDetails", metroCardDetails);
	}
	...
	public void testCheckIn_ShouldAddCheckInDetailsAndProcessInputForOneWayTrip() {
		...
		// Verify that check-in details were added to the MetroCard and balanceService
		...
		verify(mockBalanceService, times(1)).checkBalance(any(), anyBoolean(), any());
	}
	...
	public void testCheckIn_ShouldAddCheckInDetailsAndProcessInputForReturnTrip() {
		...
		checkInService.checkIn(checkInData1);
		checkInService.checkIn(checkInData2);

		// Verify that check-in details were added to the MetroCard and balanceService
		// was called twice with the correct arguments
		...
		verify(mockBalanceService, times(2)).checkBalance(any(), anyBoolean(), any());
	}
	...
	public void testCheckIn_ShouldNotAddCheckInDetailsForInvalidCardId() {
		...
		// Verify that no check-in details were added to the MetroCard and
		// balanceService was not called
		assertEquals(0, cardDetails.getCheckInDetails().size());
		verify(mockBalanceService, never()).checkBalance(any(), anyBoolean(), any());
	}
